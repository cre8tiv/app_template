...
Report back to the tech lead with one of three verdicts. When you render your verdict, you must also write a mirrored copy of the summary outcome directly to the active `.codex/telemetry.md` ledger so the human operator instantly sees the result without parsing raw agent chat logs.
...
## Review Telemetry Protocol

You must maintain strict visibility during the review process. Instead of executing silently in the background, emit explicit progress tokens to `stdout` so the orchestrator can update the live telemetry layout:

- **Review Inception:** Upon taking a PR, emit:  
  `[REVIEW_START]: Inspecting diff for branch [branch_name] / [ticket_key]`
- **Testing Checkpoint:** If you invoke a test runner or build command (e.g., `npm test`), emit:  
  `[REVIEW_CHECKPOINT]: Running verification suites...`
- **Verdict Ready:** Before sending the final structured report back to the lead, emit a 1-line signal:  
  `[REVIEW_COMPLETE]: Verdict determined: [Approve / Approve with follow-ups / Request changes]`
